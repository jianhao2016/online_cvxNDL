...
"""
This is an implementation of the paper online cvxNDL.
"""
...
def cvx_online_dict_learning(X, y_true, n_hat, k_cluster, T, lmda, eps, 
        flag=True, version = 'Rr', size_min = 5):
    '''
    X: R^(n * m)
    y_true: str^n
    W_0: R^(n_hat * k)
    ...
    t1 = time.time()
    X_hat, W_hat, representative_size_count = initialize_X_W_hat(X_0, k_cluster, size_min = size_min)
    ...
    for t in tqdm(range(T)):
        ...
        lars_lasso = Lasso(alpha = lmda, max_iter = 500)
        D_t = np.matmul(X_hat, W_hat)
    ...
    return W_hat, X_hat, representative_size_count, X_0, W_0, A_t

def initialize_X_W_hat(X_0, k_cluster, size_min = 5):
    '''
    ...
    kmeans = KMeansConstrained(n_clusters = k_cluster, size_min = size_min)
    kmeans.fit(X_0.T)
    ...
    W_hat = np.matmul(H,  np.linalg.inv(D))
...
def update_W_X_hat(W_hat, X_hat, repre_size_count, x_sample, cluster_of_x_i, 
    ...
    W_opt_old_X = opt_cal_W_hat_solve(W_hat, X_hat, A_t_inv, B_t, x_sum, alpha_sum, eps, t, cluster_seperation_idx)
    ...
    for idx in range(start_idx, end_idx):
        ...
        W_opt_new_X = opt_cal_W_hat_solve(W_hat, X_hat_new, A_t_inv, B_t, x_sum, alpha_sum, eps, t, cluster_seperation_idx)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dtype', type=str, default='scRNA',
            choices=['scRNA', 'synthetic', 'iid_ndl', 'sbm',
    ...
    parser.add_argument('--expr_name', default = 'chr4_drosophila_ChIA_Drop_0.1_PASS_10000')
    parser.add_argument('--size_min', type = int, default = 10, help='minimum representative size')
    parser.add_argument('--version', default = 'Rr', help='version of online cvxMF', choices = ['Ru', 'Rr'])
    parser.add_argument('--data_dir', default = 'processed_data/synthetic_test/', help='folder for data')
    parser.add_argument('--result_dir', default = 'results/', help='folder for output results')
    ...
    # np.random.seed(42)
    data_root_shared = args.data_dir
    if data_type == 'iid_ndl':
        expr_name = args.expr_name
        df_file = os.path.join(data_root_shared, f'df_{expr_name}')
        feat_file = os.path.join(data_root_shared, f'feature_{expr_name}')
    elif data_type == 'sbm':
        expr_name = args.expr_name
        df_file = os.path.join(data_root_shared, f'df_{expr_name}')
        feat_file = os.path.join(data_root_shared, f'feature_{expr_name}')
    else:
        raise Exception("Unsupported data type for now!")
    ...
    # test
    df_centroids_nmf.to_pickle(os.path.join(args.result_dir, f'{expr_name}_K_{k_cluster}_iter_{numIter}_nmf_centroid_df'))
    ...
    for round_num in range(aver_num):
        t1 = time.time()
        W_hat_tmp, X_hat_tmp, repre_size_count_tmp, X_0_tmp, W_0_tmp, A_t = cvx_online_dict_learning(X, Y, n_hat, k_cluster, 
                numIter, lmda, eps,
                flag = False, 
                version = args.version, size_min = args.size_min)
        t2 = time.time()
        t_ocmf += (t2 - t1)
    ...
    np.savetxt(os.path.join(args.result_dir, f'W_hat_{args.version}_{expr_name}_K_{k_cluster}_iter_{numIter}_ocmf.csv'), W_hat, delimiter = ',')
    np.savetxt(os.path.join(args.result_dir, f'A_t_{args.version}_{expr_name}_K_{k_cluster}_iter_{numIter}_ocmf.csv'), A_t, delimiter = ',')

    df_centroids = pd.DataFrame(D_final.T, columns = pca_cols)
    df_centroids['label'] = ['ocmf: type {}'.format(x) for x in range(1, k_cluster + 1)]

    df_centroids.to_pickle(os.path.join(args.result_dir, f'{args.version}_{expr_name}_K_{k_cluster}_iter_{numIter}_ocmf_centroid_df'))
    ...
    df_x_hat.to_pickle(os.path.join(args.result_dir, f'{args.version}_{expr_name}_K_{k_cluster}_iter_{numIter}_x_hat_df'))
    ...
    X = X_pca
    n_dim, m_dim = X.shape
    # D_0 = np.random.uniform(0, 1, (m_dim, k_cluster))
    # D_0 = np.absolute(D_0)
    # ipdb.set_trace()
    ...
    for round_num in range(aver_num):
        t1 = time.time()
        D_omf_final_tmp, At_omf_final_tmp = online_dict_learning(X, lmda = lmda, D_0 = D_0, T = numIter, k_cluster = k_cluster, eps = eps, _NF = _NF)
        t2 = time.time()
        t_omf += (t2 - t1)
    ...
    df_centroids_omf.to_pickle(os.path.join(args.result_dir, f'{expr_name}_K_{k_cluster}_iter_{numIter}_omf_centroid_df'))

    np.savetxt(os.path.join(args.result_dir, f'A_t_{args.version}_{expr_name}_K_{k_cluster}_iter_{numIter}_omf.csv'), At_omf_final, delimiter = ',')
    ...
    weight_col = [f'w_{x}' for x in range(W_cnmf.shape[1])]
    df_weight_cnmf = pd.DataFrame(W_cnmf, columns = weight_col)

    df_centroids_cnmf['label'] = ['CNMF: type{}'.format(x) for x in 
            range(1, k_cluster + 1)]

    df_centroids_cnmf.to_pickle(os.path.join(args.result_dir, f'{expr_name}_K_{k_cluster}_iter_{numIter}_cmf_centroid_df'))
    df_weight_cnmf.to_pickle(os.path.join(args.result_dir, f'{expr_name}_K_{k_cluster}_iter_{numIter}_cmf_weight_df'))

    # ----------------------------------------------------
    df_final = pd.DataFrame(X, columns = pca_cols)
    df_final.to_pickle(os.path.join(args.result_dir, f'{expr_name}_K_{k_cluster}_iter_{numIter}_X_df'))
