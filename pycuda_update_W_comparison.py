...
from common_functions import proj_D_onto_X_hat, cvxpy_projection_to_cvx_cmb
...
def opt_cal_W_hat_solve(W_hat, X_hat, A_t_inv, B_t, x_sum, alpha_sum, eps, t, cluster_seperation_idx = None):
    '''
    calculate optimal W_hat by setting gradient to zero, 
    ...
    for col_idx in range(W_opt.shape[1]):
        ...
        # proj_col = geo_projection_to_cvx_cmb(old_col)
        start_idx = end_idx
        end_idx = cluster_seperation_idx[col_idx]
        old_col_selected = old_col[start_idx: end_idx]

        # projection only the selected w_hat region
        proj_col = cvxpy_projection_to_cvx_cmb(old_col_selected)
        W_opt[:, col_idx] = np.zeros_like(old_col)
        W_opt[start_idx:end_idx, col_idx] = np.clip(proj_col, 0, None)
    return W_opt
