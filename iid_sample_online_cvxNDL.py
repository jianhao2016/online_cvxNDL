...
if __name__ == '__main__':
    ...
    if data_type == 'synthetic':
        ...
        df_file = os.path.join(data_root_shared, '10xGenomics_scRNA/pandasDF', df_name)
        feat_file = os.path.join(data_root_shared, '10xGenomics_scRNA/pandasDF', fc_name)
    elif data_type == 'iid_ndl':
        # p2root = '/data/shared/jianhao/online_cvxNDL_data/new_data_with_node_id'
        p2root = '/data/shared/jianhao/online_cvxNDL_data/updated_0905_data_with_node_id'
        expr_name = args.expr_name
        df_file = os.path.join(p2root, f'df_{expr_name}')
        feat_file = os.path.join(p2root, f'feature_{expr_name}')
    elif data_type == 'sbm':
        # p2root = '/data/shared/jianhao/online_cvxNDL_data/new_data_with_node_id'
        p2root = '/data/shared/jianhao/online_cvxNDL_data/synthetic_test'
        expr_name = args.expr_name
        df_file = os.path.join(p2root, f'df_{expr_name}')
        feat_file = os.path.join(p2root, f'feature_{expr_name}')
    ...
    # test
    df_centroids_nmf.to_pickle(f'results_1026_logging/{expr_name}_K_{k_cluster}_iter_{numIter}_nmf_centroid_df')
    ...
    np.savetxt(f'results_1026_logging/W_hat_{args.version}_{expr_name}_K_{k_cluster}_iter_{numIter}_ocmf.csv', W_hat, delimiter = ',')
    np.savetxt(f'results_1026_logging/A_t_{args.version}_{expr_name}_K_{k_cluster}_iter_{numIter}_ocmf.csv', A_t, delimiter = ',')

    df_centroids = pd.DataFrame(D_final.T, columns = pca_cols)
    df_centroids['label'] = ['ocmf: type {}'.format(x) for x in range(1, k_cluster + 1)]

    df_centroids.to_pickle(f'results_1026_logging/{args.version}_{expr_name}_K_{k_cluster}_iter_{numIter}_ocmf_centroid_df')
    ...
    df_x_hat.to_pickle(f'results_1026_logging/{args.version}_{expr_name}_K_{k_cluster}_iter_{numIter}_x_hat_df')
    ...
    df_centroids_omf.to_pickle(f'results_1026_logging/{expr_name}_K_{k_cluster}_iter_{numIter}_omf_centroid_df')

    np.savetxt(f'results_1026_logging/A_t_{args.version}_{expr_name}_K_{k_cluster}_iter_{numIter}_omf.csv', At_omf_final, delimiter = ',')
    ...
    df_centroids_cnmf.to_pickle(f'results_1026_logging/{expr_name}_K_{k_cluster}_iter_{numIter}_cmf_centroid_df')
    df_weight_cnmf.to_pickle(f'results_1026_logging/{expr_name}_K_{k_cluster}_iter_{numIter}_cmf_weight_df')
    # ----------------------------------------------------
    df_final = pd.DataFrame(X, columns = pca_cols)
    df_final.to_pickle(f'results_1026_logging/{expr_name}_K_{k_cluster}_iter_{numIter}_X_df')
    # df_final['label'] = Y
